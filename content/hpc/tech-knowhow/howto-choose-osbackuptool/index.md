...
各バックアップ環境は、ストレージに **ブロック・ボリューム** を使用しこれを **ベア・メタル・インスタンス** にアタッチする方法（以降"ブロック・ボリュームNFSサーバ”と呼称）でNFSでサービスするファイル共有ストレージをバックアップする前提で、何れも **[OCI HPCチュートリアル集](../../#1-oci-hpcチュートリアル集)** の **[ブロック・ボリュームでファイル共有ストレージを構築する](../../spinup-nfs-server/)** の手順に従い構築されたファイル共有ストレージと **[ベア・メタル・インスタンスNFSサーバ向けバックアップサーバを構築する](../../spinup-backup-server/)** の手順に従い構築されたバックアップ環境を使用し、ここで構築したバックアップサーバでバックアップツールを実行します。

![システム構成図OS](../../spinup-backup-server/architecture_diagram_os.png)
<center><u>バックアップ格納ストレージに <strong>オブジェクト・ストレージ</strong> を使用する場合</u></center><br>

![システム構成図BV](../../spinup-backup-server/architecture_diagram_bv.png)
...
なお、 **OCI CLI** の実行時認証は、チュートリアルに従って構築されたバックアップサーバが **[インスタンス・プリンシパル](../../#5-15-インスタンスプリンシパル)** 認証の設定を完了しているため、これを使用します。
...
なお、 **OCIFS** の実行時認証は、チュートリアルに従って構築されたバックアップサーバが **[インスタンス・プリンシパル](../../#5-15-インスタンスプリンシパル)** 認証の設定を完了しているため、これを使用します。
