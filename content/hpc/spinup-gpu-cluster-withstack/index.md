...
本チュートリアルは、 **[マーケットプレイス](../#5-5-マーケットプレイス)** から無償で利用可能な **[HPCクラスタスタック](../#5-10-hpcクラスタスタック)** を利用し、以下構成のGPUクラスタを構築、複数ノードに跨るGPU間の通信性能を **[NCCL（NVIDIA Collective Communication Library）](https://developer.nvidia.com/nccl)** の通信性能計測プログラム **[NCCL Tests](https://github.com/nvidia/nccl-tests)** で検証後、分散機械学習のサンプルプログラムを実行します。

[GPUノード]
- シェイプ ： **[BM.GPU4.8/BM.GPU.A100-v2.8](https://docs.oracle.com/ja-jp/iaas/Content/Compute/References/computeshapes.htm#bm-gpu)**
- インターコネクト ： **[クラスタ・ネットワーク](../#5-1-クラスタネットワーク)**
...
このチュートリアルで作成する環境は、前述のとおり **Slurm** と **Enroot** を使用するコンテナ環境ですが、これらの必要なソフトウェア環境は自身で整備するのでそれらを構築する際の基礎インフラストラクチャとなるGPUクラスタを構築する場合は、本チュートリアルの姉妹編である **[GPUクラスタを構築する(基礎インフラ手動構築編)](../spinup-gpu-cluster/)** や **[GPUクラスタを構築する（基礎インフラ自動構築編）](../spinup-gpu-cluster-withterraform)** を参照ください。
...
本章は、 **[HPCクラスタスタック](../#5-10-hpcクラスタスタック)** を利用し、GPUクラスタを作成します。  
**HPCクラスタスタック** は、 **[リソース・マネージャ](../#5-2-リソースマネージャ)** に作成する **[スタック](../#5-3-スタック)** からGPUクラスタを作成するため、これを許可する **IAMポリシー** が必要です。  
...
本章は、 **[リソース・マネージャ](../#5-2-リソースマネージャ)** に作成する **[スタック](../#5-3-スタック)** からGPUクラスタを作成するための **IAMポリシー** を作成します。
...
本章は、 **[HPCクラスタスタック](../#5-10-hpcクラスタスタック)** を元に、前述のGPUクラスタ環境を構築するための **スタック** を作成します。  
このチュートリアルで使用する **HPCクラスタスタック** は、バージョン **2.10.6** です。

1. 以下 **[マーケットプレイス](../#5-5-マーケットプレイス)** の **HPCクラスタスタック** ページにアクセスします。
...
本章は、完成した **[リソース・マネージャ](../#5-2-リソースマネージャ)** の **[スタック](../#5-3-スタック)** を計画し、どのようなリソースがデプロイされるか確認します。
...
本章は、計画で作成されるリソースに問題が無いことを確認した **[スタック](../#5-3-スタック)** に対し、適用を行いGPUクラスタをデプロイします。
...
3. GPUノードログイン
   ...
   GPUノードのホスト名は、Bastionノードの **/etc/opt/oci-hpc** ディレクトリ以下のファイルに格納されており、 **hostfile.tcp** と **hostfile.rdma** がそれぞれプライベートサブネット接続と **[クラスタ・ネットワーク](../#5-1-クラスタネットワーク)** サブネット接続に使用するIPアドレスに対応するホスト名です。  
...
本章は、 **[HPCクラスタスタック](../#5-10-hpcクラスタスタック)** が作成したGPUクラスタ内のLDAP統合ユーザ管理環境にLDAPユーザを作成し、このユーザでGPUクラスタにログイン後、 **Slurm** から起動するコンテナ上で簡単なコマンドが実行出来ることを確認します。
...
本章は、 **[スタック](../#5-3-スタック)** を破棄することで、構築したGPUクラスタを削除します。
