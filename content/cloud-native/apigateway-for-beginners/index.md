...
{{< hint type=note title="VCN作成済みの場合" >}}
...
{{< hint type=note title="ハンズオンで利用するAPIエンドポイント" >}}
今回のハンズオンでは単純化のために、公開されているAPIを利用して手順を作成しています。   異なる複数のエンドポイントをまとめ、一つのデプロイメントとして管理できるメリットをご体験ください。 
{{< /hint >}}
y
...
{{< hint type=note title="バックエンドサービスについて" >}}
このバックエンドサービスは米国の天気情報が取得できるサービスです。`緯度,経度`をURLで渡す必要があります。 パスの`{point}`は変数として扱うことができるため、`${request.path[point]}`の形式で取得し、バックエンドサービスに受け渡しています。 
{{< /hint >}}
...
{{< hint type=note title="バックエンドサービスについて" >}}
このバックエンドサービスはOKEハンズオンでも利用されているソースをAPIにして公開したものです。`/cowsay/say`のエンドポイントにアクセスすると牛のアスキーアートが返されます。 `${request.query[msg]}`の形式で渡した文字列をパラメータを取得しURLのクエリパラメータに利用することができます。 
{{< /hint >}}
